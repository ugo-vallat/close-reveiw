...
bool getConfigFilePath(char config_file[CONFIG_DIRECTORY_MAX_SIZE]) {
    ...
    if (getcwd(current_dir, CONFIG_DIRECTORY_MAX_SIZE) == NULL) {
        WARNL("Couldn't get current working directory")
        return false;
    }
    char *needle = strstr(current_dir, "close-review");
    if (needle == NULL) {
        WARNL("Directory 'close-review' not found in the current path")
        return false;
    }
...
Config_infos *loadConfig(Config_type type) {
    ...
    case CLIENT:
        ...
        if (config == NULL) {
            WARNL("failed to open config file at <%s>", PATH_CONFIG_CLIENT)
            return NULL;
        }
        break;
    case SERVER:
        ...
        if (config == NULL) {
            WARNL("failed to open config file at <%s>", PATH_CONFIG_SERVER)
            return NULL;
        }
        break;
    }
    ...
    if (configuration == NULL) {
        WARNL("couldn't allocate memory for config struct")
        fclose(config);
        return NULL;
    }
        ...
        if (sscanf(header, "[ %[^]] ]", token) == 1) {
            if (strncmp(token, "user", CONFIG_BUFFER_SIZE) == 0) {
                ...
                if (!configuration->user.is_defined) {
                    WARNL("Error reading user section")
                ...
                if (!configuration->server.is_defined) {
                    WARNL("Error reading server section")
                ...
                if (!configuration->history.is_defined) {
                    WARNL("Error reading history section")
                ...
                if (!configuration->config_ssl.is_defined) {
                    WARNL("Error reading ssl section")
    ...
    if (ferror(config) && !feof(config)) {
        WARNL("Error reading config file")
    }
    fclose(config);
